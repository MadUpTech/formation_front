...
import { CorrectionList, CorrectionRequest, ExamenList, Module, User } from '../modele-admin/module.model';
...
export class DetailExamenComponent implements OnInit {
  ...
  listCorrction!: Array<CorrectionList>;
  fileNam!: string;
  idModule!: number;
  idClient!: string;
  ...
  getCorrectionByClient() {
    this.adminService.getListCorrectionClient(this.idClient).subscribe({
      next: (data) => {
        this.listCorrction = data
      }, error: (err) => {
        console.log(err)
      }
    })
